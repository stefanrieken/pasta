...
#define ENGINE_INFO_PRINTF(a) printf(a)
...
const uint16_t WINDOW_ADDR_Y2 = SCREEN_HEIGHT-1;
...
void engine_display_gc9107_init(){
    ...
    // Setup GPIO
    gpio_init(PICO_DEFAULT_SPI_CSN_PIN);               // Although part of the SPI port, we'll control chip select manually
    ...
    engine_display_gc9107_apply_brightness(1.0f);  // Backlight on after all init
}

void engine_display_gc9107_update(uint16_t *screen_buffer_to_render) {
    if(dma_channel_is_busy(dma_tx)){
        ENGINE_INFO_PRINTF("Waiting on previous DMA transfer to complete. Could have done more last frame!");
    ...
    dma_channel_configure(dma_tx, &dma_config,
                          ...
                          txbuf,                        // read address
                          SCREEN_WIDTH * SCREEN_HEIGHT,    // element count (each element is of size DMA_SIZE_16)
